...
function regimeSection(macro) {
  if (!macro || !Object.keys(macro).length) {
    return [
      isAdvisoryMode() ? "Market regime **unavailable** \u2014 run `npm run ingest:macro` or `npm run ingest:daily` before sizing positions." : "Market regime: not supplied.",
      null
    ];
  }
  const rating = macro.rating ?? "unknown";
  const mult = macro.risk_multiplier;
  const mtxt = mult != null ? ` \xB7 risk multiplier **${mult}**` : "";
  const reasoning = macro.reasoning;
  let line = `Market regime: **${rating}**${mtxt}. ` + (isAdvisoryMode() ? "Scale position sizes and new entries accordingly." : "Position-sizing conditions scale with this multiplier.");
  if (reasoning?.length) {
...
function positionsSection(positions) {
  ...
  for (const p of positions) {
    ...
    if (ds != null && ds <= NEAR_PCT) {
      const action = isAdvisoryMode() ? ` Review stop at \u09F3${stop} \u2014 price \u09F3${px} is ${ds.toFixed(1)}% away.` : "";
      ...
      const action = isAdvisoryMode() ? ` Consider taking partial profits near target \u09F3${tgt}.` : "";
  ...
  if (!lines.length) {
    lines.push(
      isAdvisoryMode() ? "No held position is within ~3% of a stop or target \u2014 no immediate level alerts." : "No held position is within ~3% of a stop or target level."
    );
  }
  return [lines, items];
}
...
function signalSummarySection(positions, watch) {
  ...
  if (positions.length) {
    ...
    for (const p of positions) {
      ...
      lines.push(
        `- **${t}**${pxTxt}: Investment **${formatRating(inv)}**${formatScore(p.investment_score)} \xB7 Momentum **${formatRating(mom)}**${formatScore(p.momentum_score)}`
      );
  ...
  if (watch.length) {
    ...
    for (const w of watch) {
      ...
      const pxTxt = px != null ? ` @ \u09F3${px}` : "";
      const entryTxt = entry != null ? ` \xB7 entry zone \u09F3${entry}` : "";
      lines.push(
        `- **${t}** (${purpose})${pxTxt}: Inv **${formatRating(inv)}**${formatScore(w.investment_score)} \xB7 Mom **${formatRating(mom)}**${formatScore(w.momentum_score)}${entryTxt}`
      );
...
function calendarSection(calendar, asOf) {
  ...
  if (!lines.length) {
    lines.push(
      isAdvisoryMode() ? "No economic or earnings events dated today \u2014 check DSE price-sensitive notices separately." : "No economic or earnings events are dated today."
    );
  }
  return [lines, items];
}
function newsSection(news, knownTickers) {
  ...
  if (!lines.length) {
    lines.push(
      isAdvisoryMode() ? "No overnight news in the last 7 days \u2014 run `npm run ingest:daily` or `npm run ingest:news` if feeds are stale." : "No overnight news or disclosures supplied."
    );
  }
  return [lines, items];
}
function riskSection(positions, news, macro) {
  ...
  if (book > 0) {
    for (const [t, v] of vals) {
      ...
      if (wpct >= CONCENTRATION_PCT) {
        const action = isAdvisoryMode() ? ` Consider trimming ${t} or adding hedges \u2014 above ${CONCENTRATION_PCT.toFixed(0)}% single-name limit.` : "";
        lines.push(
  ...
  for (const n of news) {
    if (kw.test(String(n.headline ?? ""))) {
      lines.push(
        isAdvisoryMode() ? `Microstructure alert: ${n.ticker ?? "?"} \u2014 circuit/floor/halt mentioned; avoid new entries until normal trading.` : `Microstructure note: ${n.ticker ?? "?"} headline mentions a circuit/floor/halt condition.`
      );
  ...
  if (["risk_off", "cautious", "bearish", "red"].includes(regime)) {
    lines.push(
      isAdvisoryMode() ? `Macro regime is **${regime}** \u2014 reduce new position sizes and tighten stops.` : "Macro regime is risk-off; sizing conditions are tighter than usual."
    );
    items.push({ type: "macro_risk_off", regime });
  }
  if (!lines.length) {
    lines.push(
      isAdvisoryMode() ? "No elevated concentration or microstructure risks detected." : "No elevated risk items detected in the supplied data."
    );
  }
  return [lines, items];
}
